...
const AddAutoRouterTab: React.FC<AddAutoRouterTabProps> = ({
  ...
  const {
    ...
    queryKey: ["availableModels", "autoRouter", accessToken, userId ?? "", userRole],
    queryFn: async () => {
      const [groups, deployments] = await Promise.all([
        fetchAvailableModels(accessToken),
        fetchAllModelDeployments(accessToken, userId ?? "", userRole).catch(() => []),
      ]);
      return { groups, deployments };
    },
    enabled: Boolean(accessToken),
  });
  const modelInfo = React.useMemo(() => data?.groups ?? [], [data]);
  ...
  const availability = React.useMemo(
    () =>
      buildModelAvailability(
        ...
        deploymentRefsFromModelInfo(data?.deployments ?? []),
      ),
    [modelInfo, data],
  );
